...
from .forms import LoginForm, StudentForm, UpdateStudentForm, FacultyForm
...
class StudentScheduleList(ListView):
    ...
    def get_queryset(self, *args, **kwargs):
        qs = super(StudentScheduleList, self).get_queryset(*args, **kwargs)
        qs = qs.order_by("year_section")
        if self.request.GET.get("q") != None:
            query = self.request.GET.get('q')
            qs = qs.order_by("year_section").filter(Q(year_section__icontains=query)| Q(block_section__icontains=query))
        return qs
